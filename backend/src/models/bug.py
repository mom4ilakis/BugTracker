...
class Bug(TimeStampMixin, table=True):
    ...
    priority: Priority = Field(
        sa_column=Column(SQLEnum(Priority), default=Priority.MEDIUM, server_default=Priority.MEDIUM.value,
                         nullable=False))
    severity: Severity = Field(
        sa_column=Column(SQLEnum(Severity), default=Severity.MEDIUM, server_default=Severity.MEDIUM.value,
                         nullable=False))
    reported_by: int | None = Field(default=None, foreign_key="user.id")
    reporter: User = Relationship(back_populates="bugs", sa_relationship_kwargs={"foreign_keys": "Bug.reported_by"})
    assigned_to: int | None = Field(default=None, foreign_key="user.id")
    assignee: User | None = Relationship(back_populates="assigned_bugs",
                                         sa_relationship_kwargs={"foreign_keys": "Bug.assigned_to"})
